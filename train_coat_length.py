...
model_dir = "model/model_coat_length"
logs_dir = "./logs/coat_length"
label_dir = './Annotations/coat_length_labels.csv'
print_dir = './logs/logs_coat_length.txt'
...
for epoch in range(15):
    ...
    for idxs in range(batch_idxs):
        ...
        if step % 1 == 0:
            print("Epoch: [%3d/%3d] [%4d/%4d] avgtime %4.4f, train_loss: %.8f, train_acc: %.4f" % (epoch, 15, idxs, batch_idxs, time.time()-start_time, trainloss, trainacc))
            print("Epoch: [%3d/%3d] [%4d/%4d] avgtime %4.4f, train_loss: %.8f, train_acc: %.4f" % (epoch, 15, idxs, batch_idxs, time.time() - start_time, trainloss, trainacc), file=outlogs)
